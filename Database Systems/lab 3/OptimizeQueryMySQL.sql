...
WITH
  periods AS (
    ...
    SELECT
      ...
      '2020-11-09' +
        INTERVAL DATEDIFF('2020-11-09', '2020-11-15') -1 day
      AS start_date,
      '2020-11-15' +
        INTERVAL DATEDIFF('2020-11-09', '2020-11-15') -1 day
      AS end_date
  ...
  single_number_summary_values AS (
    SELECT
      cur_period.start_date AS cur_start_date,
      cur_period.end_date AS cur_end_date,
      ref_period.start_date AS ref_start_date,
      ref_period.end_date AS ref_end_date,
  ...
  average_time_spent_int AS (
    SELECT round(cur_average_time_on_page) AS avg_time_spent
  ...
  average_time_spent_strings_split AS (
    SELECT
      avg_time_spent,
      CASE
        WHEN minutes > 0 THEN CONCAT(CAST(minutes AS CHAR(20)), 'm')
        ELSE ''
      END as minutes_str,
      CASE
        WHEN seconds > 0 OR minutes=0 THEN CONCAT(CAST(seconds AS CHAR(20)), 's')
        ELSE ''
      END as seconds_str
  ...
  average_time_spent AS (
    SELECT
      avg_time_spent,
      CASE
        WHEN minutes_str != '' AND seconds_str != '' THEN CONCAT(minutes_str, ' ', seconds_str)
  ...
  SELECT
    ...
    CASE
      ...
      ELSE (cur_average_time_on_page - ref_average_time_on_page)/ref_average_time_on_page*100.0
    END  AS average_time_spent_percent_change,
    CASE
      WHEN cur_total_views=0 THEN NULL
      ELSE cur_total_views
    END AS total_views,
    CASE
      WHEN cur_total_views=0 THEN NULL
      WHEN ref_total_views=0 THEN 0
      ELSE (cur_total_views - ref_total_views)/ref_total_views*100.0
    END  AS total_views_percent_change,
    CASE
      WHEN cur_total_views=0 THEN NULL
    ...
    CASE
      ...
      ELSE (cur_new_user_page_views - ref_new_user_page_views)/ref_new_user_page_views*100.0
    END  AS new_user_page_views_percent_change,
    CASE
      WHEN cur_total_views=0 THEN NULL
      ELSE cur_returning_user_page_views
    END  AS returning_user_page_views,
    CASE
      WHEN cur_total_views=0 THEN NULL
      WHEN ref_returning_user_page_views=0 THEN 0
      ELSE (cur_returning_user_page_views - ref_returning_user_page_views)/ref_returning_user_page_views*100.0
    END  AS returning_user_page_views_percent_change
...
WITH listing_parameters AS (
  ...
  SELECT
    ...
    CASE
      ...
      WHEN 'all-listings' IS NULL OR 'all-listings' LIKE 'active%'
        OR 'all-listings' LIKE 'Coming Soon%' THEN TRUE
      ELSE FALSE
...
SELECT DISTINCT
  ...
  CASE
    ...
    ELSE CONCAT(props.pretty_address, ' (Rental)')
  END AS pretty_address,
  CASE
    WHEN props.is_sales_listing THEN props.display_address
    ELSE CONCAT(props.display_address, ' (Rental)')
...
--          ),
			...
			other_publisher_views_by_period AS (
						 ...
						 FROM agent_insights_lt.agent_insights__combined_listing_views AS clv, periods AS periods
						WHERE clv.date BETWEEN periods.start_date AND periods.end_date
					 ),


         single_number_summary_values AS (
             SELECT cur_period.start_date                                                                AS cur_start_date,
                    cur_period.end_date                                                                  AS cur_end_date,
                    ref_period.start_date                                                                AS ref_start_date,
                    ref_period.end_date                                                                  AS ref_end_date,
    ...
    SELECT CASE
           ...
           CASE
               WHEN cur_total_views = 0 THEN NULL
               WHEN ref_average_time_on_page = 0 THEN 0
               ELSE (cur_average_time_on_page - ref_average_time_on_page) /
           ...
           CASE
               WHEN cur_total_views = 0 THEN NULL
               WHEN ref_total_views = 0 THEN 0
               ELSE (cur_total_views - ref_total_views) / (ref_total_views + (1e-10)) * 100.0
               END  AS total_views_percent_change,
           CASE
               WHEN cur_total_views = 0 THEN NULL
               ELSE cur_new_user_page_views
           ...
           CASE
               WHEN cur_total_views = 0 THEN NULL
               WHEN ref_new_user_page_views = 0 THEN 0
               ELSE (cur_new_user_page_views - ref_new_user_page_views) / (ref_new_user_page_views + (1e-10)) *
                    100.0
               END  AS new_user_page_views_percent_change
    FROM single_number_summary_values,
         average_time_spent
